...
ny_values=(1 2 3 4)
nz_values=(1 2 3 4)
average_threshold_values=(-1 0.2 0.3 0.4 0.5)
ipmv_values=(0.25 0.2 0.3 0.4 0.5)
infer_with_max_filter_fun=(true false)
        ...
        for average_threshold in "${average_threshold_values[@]}"; do
          for ipmv in "${ipmv_values[@]}"; do
            for max_filter_fun in "${infer_with_max_filter_fun[@]}"; do
              ...
              cmd="python main.py infer $config -v $test_data -m $model -nx $nx -ny $ny -nz $nz --average_threshold $average_threshold -ipmv $ipmv"
